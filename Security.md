...
- The service runs under a least-privilege **virtual service account** (`NT SERVICE\Jenkins`) by default, and the runtime data folder is granted to that account explicitly (inheritable `Modify`, with inheritance left on so SYSTEM and Administrators keep full control)
  - **Fixed in this release:** installs performed by the `.exe` bundle registered the service as **LocalSystem** instead. The bundle forwarded an empty `SERVICE_ACCOUNT`, and an empty public property on the msiexec command line *overrides* the package default rather than deferring to it, so the service was registered with no account name at all. Upgrading with a current `.exe` corrects the identity in place, keeping the configuration, secret and data folder. To check an existing install: `(Get-CimInstance Win32_Service -Filter "Name='Jenkins'").StartName`
