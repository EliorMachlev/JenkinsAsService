...
- **Not yet verified against real infra:** a real MSI install validating ACL/TPM-decrypt; live WebSocket/`Auto`-fallback against a controller; and the `Agent:LaunchInInteractiveSession` `CreateProcessAsUser` path (needs a box with autologon + the service set to LocalSystem — same untestable-in-CI class as the TPM code).
