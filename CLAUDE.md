...
- **Agent process:** `AgentProcessLauncher.cs` (the `IAgentProcessLauncher`/`IAgentProcess` seam — production wraps `System.Diagnostics.Process`; fakes drive the watchdog in tests), `InteractiveSessionLauncher.cs` (opt-in `Agent:LaunchInInteractiveSession` — `CreateProcessAsUser` into the console session for visible GUI/browser tests; off by default, warn+fall-back on any precondition miss; enables `RequiredPrivileges` (`SeTcbPrivilege`/`SeAssignPrimaryTokenPrivilege`/`SeIncreaseQuotaPrivilege`) in its own token first, since URA grants land *present but disabled* and only LocalSystem gets them pre-enabled; **interop is compile-verified only, not exercised by CI**), `AgentArgumentParser.cs`, `AgentTransport.cs` (pure transport-selection: `InitialMethod`/`Toggle`/`ShouldFallback`), `JavaPathResolver.cs`, `ServiceSettingsValidator.cs`
...
- **Not yet verified against real infra:** ACL/TPM-decrypt on a real install; the `Auto` transport *fallback toggle* actually firing (fresh-install Auto connected, but not observed falling WebSocket↔TCP); the MSI upgrade path (`WIX_UPGRADE_DETECTED` gating + `UpgradeConfig` CA) — the reconcile/preserve logic is unit-tested, but CA sequencing against a live in-place upgrade is manual-verify; and the `Agent:LaunchInInteractiveSession` `CreateProcessAsUser` path (needs a box with autologon + the service set to LocalSystem). The named-account path was confirmed broken on a real box (repeated `ERROR_PRIVILEGE_NOT_HELD` from `WTSQueryUserToken` despite `SeTcbPrivilege` verified assigned via `secedit /export /areas USER_RIGHTS`); the `AdjustTokenPrivileges` fix is the response, but the *success* case is still unverified against real infra — all the same untestable-in-CI class as the TPM code.
