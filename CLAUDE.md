...
- **Not yet verified against real infra:** a real MSI install validating ACL/TPM-decrypt, live WebSocket/`Auto`-fallback against a controller, and the MSI upgrade path (`WIX_UPGRADE_DETECTED` gating + `UpgradeConfig` CA) — the reconcile/preserve logic is unit-tested, but CA sequencing against a live in-place upgrade is manual-verify, same class as TPM/ACL.
