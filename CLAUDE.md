...
- **Secrets:** `ISecretResolver.cs`/`SecretResolver.cs`, `SecretWriter.cs`, `UpdateSecretCommand.cs` (the `update-secret` CLI — dispatch, arg parsing, silent/upgrade flows), `InteractiveConfigPrompts.cs` (console prompt primitives for the interactive flow, incl. masked input), `ImpersonationRunner.cs` (`LogonUser` + `RunImpersonated` for Credential-Manager/User-DPAPI writes; isolates the native `extern` surface), `TpmSecretProtector.cs`, `AgentSecretFile.cs` (`-secret @<file>` off-argv), `CertificateThumbprintValidator.cs`
- **Agent process:** `AgentProcessLauncher.cs` (the `IAgentProcessLauncher`/`IAgentProcess` seam — production wraps `System.Diagnostics.Process`; fakes drive the watchdog in tests), `InteractiveSessionLauncher.cs` (opt-in `Agent:LaunchInInteractiveSession` — `CreateProcessAsUser` into a logged-on user's session for visible GUI/browser tests; off by default, warn+fall-back on any precondition miss; enables `RequiredPrivileges` (`SeTcbPrivilege`/`SeAssignPrimaryTokenPrivilege`/`SeIncreaseQuotaPrivilege`) in its own token first, since URA grants land *present but disabled* and only LocalSystem gets them pre-enabled; **interop is compile-verified only, not exercised by CI**), `InteractiveSessionSelector.cs` (pure session-selection policy — enumerate-and-choose, unit-tested; see the session-selection invariants below), `AgentArgumentParser.cs`, `AgentTransport.cs` (pure transport-selection: `InitialMethod`/`Toggle`/`ShouldFallback`), `JavaPathResolver.cs`, `ServiceSettingsValidator.cs`
...
- **Interactive-session targeting:** `InteractiveSessionSelector` enumerates sessions and picks one; it does **not** use `WTSGetActiveConsoleSessionId` (that returns the physical console even when it is empty at the logon screen, and cannot see an RDP session — the cause of a real-node `ERROR_NO_TOKEN`). A **logged-on user**, not the connect state, is the signal a token exists; **disconnected sessions are first-class targets** (closing an RDP window keeps the user, desktop and running browser alive) — don't "fix" the selector to prefer only `Active`. Session 0 is never a target. `RequireInteractiveSession` converts the headless fallback into a throw so the bring-up loop retries; since bring-up failures never count toward `MaxRetries`, waiting for a desktop can't fault the service.
- **Selector ranking is three ordered keys** — connect state (per `PreferDisconnectedSession`) → **console last** → session id (lowest, or highest under `PreferDisconnectedSession`). The console key must stay *below* the state key: it exists so an autologon console desktop is the fallback rather than the permanent winner of the id tie-break, but promoting it above the state rank would let an operator's freshly-signed-in `Active` RDP session steal the agent, which is exactly what `PreferDisconnectedSession` exists to prevent. Console-ness comes from the WinStation name (`Console` vs `RDP-Tcp#N`), never from "session 1".
- **Session migration restarts the agent.** `SessionMigration` (`Off`/`OnSessionLost`/`Always`) re-runs selection once per stability window from the *still-running* branch of the supervision loop. A process cannot move between sessions, so migration is kill + relaunch: the node drops and any in-flight build dies — hence `Off` by default. The kill deliberately happens **before** the exit is observed, so a migration never reaches the crash-counting path and can't burn `MaxRetries`; `bringUpAttempts` is reset so no backoff is applied. `InteractiveSessionSelector.ShouldMigrate` is pure and returns no replacement session id on purpose — the relaunch re-runs full bring-up, including the headless Session 0 fallback. `IAgentProcess.InteractiveSessionId` is null for a Session 0 launch, which is what makes the "upgrade a headless agent" case expressible.
- **An interactive launch runs the agent as the *session user*, not the service.** `CreateProcessAsUser` doesn't build an environment for the token (NULL inherits the *caller's*), so the child env comes from `CreateEnvironmentBlock` on the target token — machine + that user's variables, OS-resolved profile paths — with `SanitizeEnvironment`'s allow-list applied to *that* block (filtering is by name, so the posture is unchanged). Without it a LocalSystem service hands the agent `USERPROFILE=C:\Windows\system32\config\systemprofile`. `InteractiveLaunchAcl` then grants the session user read on the secret file (it reads `-secret @file` itself) and `Modify` on `work\` + read-only on `agent\` — **never write on `agent\` or the data root**, or a build step could clobber `agent.jar`, the logs or the secret. All best-effort, all no-ops when service and session user match.
- **An ineffective migration must not repeat.** `SessionMigrationTracker` (worker state, unit-tested) remembers the `(agent session, rendered session table)` a migration was ordered from and refuses an identical repeat, logging once. Because the relaunch falls back to headless whenever the interactive launch fails, `Always` on a box where that launch *can't* succeed (wrong identity, `ERROR_ACCESS_DENIED`) otherwise sees "a desktop exists, upgrade the headless agent" forever — dropping the node and killing the running build every stability window, with `MaxRetries` unable to stop it because migrations are off the crash path. The guard self-clears: any change to either half of the origin is a new state.
...
- Key axes: `Connection:Method` (`Auto`/`WebSocket`/`Https` — `Https` means direct TCP inbound, not literal HTTPS), `Secret:Mode` (`Unprotected`/`Dpapi`/`Tpm`/`EnvironmentVariable`/`CredentialManager`), `Secret:ViaFile`, `Secret:DpapiScope`, `Hardening:SanitizeEnvironment`, `Agent:DataDirectory`, `Agent:LaunchInInteractiveSession:{Enabled,LocalSystemOnly,TargetUser,RequireInteractiveSession,PreferDisconnectedSession,SessionMigration}` (opt-in Session-0→interactive-session launch; off by default, LocalSystem-gated. `LocalSystemOnly: false` needs all three privileges granted via URA **and a service restart** — tokens take privileges at logon; note URA is winner-takes-all, so a domain GPO defining any of them silently reverts a local grant. **The privileges alone are not sufficient**: `WTSQueryUserToken` also runs a termsrv access check on the target session, which a low-priv account fails with `ERROR_ACCESS_DENIED (5)` even with all three rights enabled — confirmed on a real node, and fixed by switching the same service to LocalSystem. A named account must also be an Administrators member; prefer LocalSystem. See security posture note below), `Recovery:MaxRetries` (0 = infinite).
...
- Recent merges: hardening batch + shutdown-race fixes (PR #47/#48); full installer config coverage (PR #50, tag 1.7); Auto-transport fallback made to actually fire via `-noReconnect` (PR #57, tag 1.12); jar SHA-256 integrity check + `agent\`/`work\` data-dir split (PR #58); opt-in interactive-session launch for visible GUI/browser tests (PR #69).
...
- **Not yet verified against real infra:** ACL/TPM-decrypt on a real install; the `Auto` transport *fallback toggle* actually firing (fresh-install Auto connected, but not observed falling WebSocket↔TCP); the MSI upgrade path (`WIX_UPGRADE_DETECTED` gating + `UpgradeConfig` CA) — the reconcile/preserve logic is unit-tested, but CA sequencing against a live in-place upgrade is manual-verify; and the `Agent:LaunchInInteractiveSession` `CreateProcessAsUser` path (needs a box with autologon + the service set to LocalSystem). The named-account path was confirmed broken on a real box in two successive stages: first `ERROR_PRIVILEGE_NOT_HELD (1314)` despite `SeTcbPrivilege` verified assigned (fixed by `AdjustTokenPrivileges` — URA grants land disabled), then `ERROR_ACCESS_DENIED (5)` from termsrv's own access check on the target session, which a low-priv account fails regardless of privileges. Switching the same service to LocalSystem cleared the 5 and surfaced `ERROR_NO_TOKEN (1008)` — the console session was empty at the logon screen while the operator worked over RDP, which is what motivated `InteractiveSessionSelector`. **Conclusion: run as LocalSystem; `LocalSystemOnly: false` additionally requires Administrators membership.** The end-to-end *success* case (agent visibly launched on a user desktop) is still unverified against real infra — same untestable-in-CI class as the TPM code.
